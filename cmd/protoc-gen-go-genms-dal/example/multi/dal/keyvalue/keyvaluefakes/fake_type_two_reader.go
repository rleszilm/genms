// Code generated by counterfeiter. DO NOT EDIT.
package keyvaluefakes

import (
	"context"
	"sync"

	"github.com/rleszilm/genms/cmd/protoc-gen-go-genms-dal/example/multi"
	keyvalue_dal_multi "github.com/rleszilm/genms/cmd/protoc-gen-go-genms-dal/example/multi/dal/keyvalue"
)

type FakeTypeTwoReader struct {
	GetByKeyStub        func(context.Context, keyvalue_dal_multi.TypeTwoKey) (*multi.TypeTwo, error)
	getByKeyMutex       sync.RWMutex
	getByKeyArgsForCall []struct {
		arg1 context.Context
		arg2 keyvalue_dal_multi.TypeTwoKey
	}
	getByKeyReturns struct {
		result1 *multi.TypeTwo
		result2 error
	}
	getByKeyReturnsOnCall map[int]struct {
		result1 *multi.TypeTwo
		result2 error
	}
	invocations      map[string][][]interface{}
	invocationsMutex sync.RWMutex
}

func (fake *FakeTypeTwoReader) GetByKey(arg1 context.Context, arg2 keyvalue_dal_multi.TypeTwoKey) (*multi.TypeTwo, error) {
	fake.getByKeyMutex.Lock()
	ret, specificReturn := fake.getByKeyReturnsOnCall[len(fake.getByKeyArgsForCall)]
	fake.getByKeyArgsForCall = append(fake.getByKeyArgsForCall, struct {
		arg1 context.Context
		arg2 keyvalue_dal_multi.TypeTwoKey
	}{arg1, arg2})
	fake.recordInvocation("GetByKey", []interface{}{arg1, arg2})
	fake.getByKeyMutex.Unlock()
	if fake.GetByKeyStub != nil {
		return fake.GetByKeyStub(arg1, arg2)
	}
	if specificReturn {
		return ret.result1, ret.result2
	}
	fakeReturns := fake.getByKeyReturns
	return fakeReturns.result1, fakeReturns.result2
}

func (fake *FakeTypeTwoReader) GetByKeyCallCount() int {
	fake.getByKeyMutex.RLock()
	defer fake.getByKeyMutex.RUnlock()
	return len(fake.getByKeyArgsForCall)
}

func (fake *FakeTypeTwoReader) GetByKeyCalls(stub func(context.Context, keyvalue_dal_multi.TypeTwoKey) (*multi.TypeTwo, error)) {
	fake.getByKeyMutex.Lock()
	defer fake.getByKeyMutex.Unlock()
	fake.GetByKeyStub = stub
}

func (fake *FakeTypeTwoReader) GetByKeyArgsForCall(i int) (context.Context, keyvalue_dal_multi.TypeTwoKey) {
	fake.getByKeyMutex.RLock()
	defer fake.getByKeyMutex.RUnlock()
	argsForCall := fake.getByKeyArgsForCall[i]
	return argsForCall.arg1, argsForCall.arg2
}

func (fake *FakeTypeTwoReader) GetByKeyReturns(result1 *multi.TypeTwo, result2 error) {
	fake.getByKeyMutex.Lock()
	defer fake.getByKeyMutex.Unlock()
	fake.GetByKeyStub = nil
	fake.getByKeyReturns = struct {
		result1 *multi.TypeTwo
		result2 error
	}{result1, result2}
}

func (fake *FakeTypeTwoReader) GetByKeyReturnsOnCall(i int, result1 *multi.TypeTwo, result2 error) {
	fake.getByKeyMutex.Lock()
	defer fake.getByKeyMutex.Unlock()
	fake.GetByKeyStub = nil
	if fake.getByKeyReturnsOnCall == nil {
		fake.getByKeyReturnsOnCall = make(map[int]struct {
			result1 *multi.TypeTwo
			result2 error
		})
	}
	fake.getByKeyReturnsOnCall[i] = struct {
		result1 *multi.TypeTwo
		result2 error
	}{result1, result2}
}

func (fake *FakeTypeTwoReader) Invocations() map[string][][]interface{} {
	fake.invocationsMutex.RLock()
	defer fake.invocationsMutex.RUnlock()
	fake.getByKeyMutex.RLock()
	defer fake.getByKeyMutex.RUnlock()
	copiedInvocations := map[string][][]interface{}{}
	for key, value := range fake.invocations {
		copiedInvocations[key] = value
	}
	return copiedInvocations
}

func (fake *FakeTypeTwoReader) recordInvocation(key string, args []interface{}) {
	fake.invocationsMutex.Lock()
	defer fake.invocationsMutex.Unlock()
	if fake.invocations == nil {
		fake.invocations = map[string][][]interface{}{}
	}
	if fake.invocations[key] == nil {
		fake.invocations[key] = [][]interface{}{}
	}
	fake.invocations[key] = append(fake.invocations[key], args)
}

var _ keyvalue_dal_multi.TypeTwoReader = new(FakeTypeTwoReader)
