...
func (x *SingleCollection) DoInsert(ctx context.Context, arg *single.Single) (sql1.Result, error) {
	...
	ctx, _ = tag.New(ctx,
		tag.Upsert(tagQueryCollection, "single"),
	)
	...
	return x.db.ExecWithReplacements(ctx, x.execInsert, singleWriterFromGeneric(arg))
}
...
func (x *SingleCollection) DoUpsert(ctx context.Context, arg *single.Single) (sql1.Result, error) {
	...
	return x.db.ExecWithReplacements(ctx, x.execUpsert, singleWriterFromGeneric(arg))
}
...
func (x *SingleCollection) DoUpdate(ctx context.Context, fvs *dal.SingleFieldValues, clause string) (sql1.Result, error) {
	...
	return x.db.ExecWithReplacements(ctx, string(buf.Bytes()), singleFieldValuesFromGeneric(fvs))
}
...
func NewSingleCollection(db sql.DB, queries SingleQueryTemplateProvider, config *SingleConfig) (*SingleCollection, error) {
	registerSingleMetricsOnce.Do(registerSingleMetrics)
...
var (
	...
	measureLatency  = stats.Float64("dal_postgres_latency", "Latency of Single queries", stats.UnitMilliseconds)
	measureInflight = stats.Int64("dal_postgres_inflight", "Count of Single queries in flight", stats.UnitDimensionless)

	registerSingleMetricsOnce sync.Once
)

func registerSingleMetrics() {
	views := []*view.View{
		{
