...
func NewUserCollection(db sql.DB, config *UserConfig) *UserCollection {
	...
	return &UserCollection{
		...
		db:                          db,
		config:                      config,
		queryAll:                    "SELECT id, name, division, lifetime_score, last_score, payout FROM " + config.TableName,
		queryById: `SELECT id, name, division, lifetime_score, last_score, payout FROM ` + config.TableName +
			`WHERE 
					id = :id AND
					1 = 1;`,
		queryByNameAndDivision: `SELECT id, name, division, lifetime_score, last_score, payout FROM ` + config.TableName +
			`WHERE 
					name = :name AND
