...
type {{ MessageName .msg }}CollectionReader interface {
	...
	Filter({{ .P.Context }}.Context, *{{ MessageName .msg }}Filter) ([]*{{ QualifiedType .outfile .msg }}, error)
	{{ range .opts.Queries -}}
...
// {{ MessageName .msg }}Filter is an autogenerated struct that can be used in the generic queries against {{ MessageName .msg }}Collection.
type {{ MessageName .msg }}Filter struct {
	{{ range .fields.Names -}}
		{{- $qName := ($state.fields.QueryNameByFieldName .) -}}
		{{- if $qName -}}
