...
// KeyValuePackageName returns the name of the dal  package.
func (f *File) KeyValuePackageName() string {
	return "keyvalue_" + f.File.DalPackageName()
}

// KVPackagePath returns the path of the package.
func (f *File) KVPackagePath() string {
	toks := append([]string{strings.ReplaceAll(f.Proto().GoImportPath.String(), "\"", "")}, "dal", "keyvalue")
