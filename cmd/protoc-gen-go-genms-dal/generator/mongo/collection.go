...
func (x *{{ $.C.Message.Name }}Collection) Find(ctx {{ $.P.Context }}.Context, label string, filter {{ $.P.Bson }}.M, opts ...*{{ $.P.Mongo }}.FindOptions) ([]*{{ $.C.Message.QualifiedKind }}, error) {	
	...
	if err != nil {
		{{ $.P.Mongo }}.Logs().Error("could not dial:", err)
	...
	for cur.Next(ctx) {
		...
		if err != nil {
			{{ $.P.Mongo }}.Logs().Error("could not convert from mongo to internal:", err)
			{{ $.P.Stats }}.Record(ctx, {{ $.P.Mongo }}.MeasureError.M(1))
			return nil, err
		}
		...
		for _, m := range x.mutators {
			val, err = m(val)
			if err != nil {
				{{ $.P.Mongo }}.Logs().Error("could not mutate value:", val, err)
				{{ $.P.Stats }}.Record(ctx, {{ $.P.Mongo }}.MeasureError.M(1))
				return nil, err
			}
		}
		...
		{{- if not $f.Ignore -}}
			...
			if fvs.{{ ToTitleCase $f.Name }} != nil {
				{{ if eq $conv "ObjectID" -}}
					...
					if err != nil {
						{{ $.P.Mongo }}.Logs().Error("could not convert value to ObjectID:", err)
						return nil, err
					}
			...
			{{- if not $f.Ignore -}}
				...
				if fvs.{{ ToTitleCase $f.Name }} != nil {
					{{ if eq $conv "ObjectID" -}}
						conv{{ ToTitleCase $f.Name }}, err := {{ $.P.Bson }}.ToObjectID({{- if not $f.IsRef -}}*{{- end -}}fvs.{{ ToTitleCase $f.Name }})
						if err != nil {
							{{ $.P.Mongo }}.Logs().Error("could not convert to ObjectID:", {{- if not $f.IsRef -}}*{{- end -}}fvs.{{ ToTitleCase $f.Name }}, err)
							{{ $.P.Stats }}.Record(ctx, {{ $.P.Mongo }}.MeasureError.M(1))
							return nil, err
						}
		...
		func (x *{{ $.C.Message.Name }}Collection){{ ToTitleCase $q.Name }}(ctx {{ $.P.Context }}.Context
			...
			{{ range $a := $q.Args }}
				...
				{{ if eq $conv "ObjectID" -}}
					...
					if err != nil {
						{{ $.P.Mongo }}.Logs().Error("could not convert to ObjectID:", {{ $arg.Name }}, err)
						{{ $.P.Stats }}.Record(ctx, {{ $.P.Mongo }}.MeasureError.M(1))
						return nil, err
					}
