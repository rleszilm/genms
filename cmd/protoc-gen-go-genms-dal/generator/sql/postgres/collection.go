...
func NewCollection(plugin *protogen.Plugin, file *protogen.File, msg *protogen.Message, opts *annotations.DalOptions) *Collection {
	...
	cfile := generator.NewFile(outfile, file)
	cmsg := generator.NewMessage(cfile, msg)
...
func (c *Collection) defineCollection() error {
	...
	`
		...
		p := map[string]string{
			...
			"GenMSSQL":   generator.QualifiedPackageName(c.Outfile, "github.com/rleszilm/genms/sql"),
		}
...
func (c *Collection) defineDefaultQueries() error {
	...
	func (x *{{ MessageName .C.Message }}Collection) Filter(ctx {{ .P.Context }}.Context, arg *{{ QualifiedDalType .C.Outfile .C.Message }}Filter) ([]*{{ QualifiedType .C.Outfile .C.Message }}, error) {
	...
	func (x *{{ MessageName .C.Message }}Collection) find(ctx {{ .P.Context }}.Context, label string, query string, arg *{{ QualifiedDalType .C.Outfile .C.Message }}Filter) ([]*{{ QualifiedType .C.Outfile .C.Message }}, error) {
		var err error
		start := {{ .P.Time }}.Now()
		{{ .P.Stats }}.Record(ctx, {{ ToCamelCase (MessageName .C.Message) }}Inflight.M(1))
		defer func() {
		...
		filter := &{{ MessageName .C.Message }}Filter{}
...
func (c *Collection) defineNewCollection() error {
	...
	`
		...
		p := map[string]string{
			"Collection":  generator.QualifiedPackageName(c.Outfile, generator.DalPackagePath(c.File)),
			"GenMSSQL":    generator.QualifiedPackageName(c.Outfile, "github.com/rleszilm/genms/sql"),
			"GenerateSQL": generator.QualifiedPackageName(c.Outfile, "github.com/rleszilm/genms/cmd/protoc-gen-go-genms-dal/generator/sql"),
		}
...
func (c *Collection) defineInternalStructs() error {
	/*tmplSrc := `// {{ MessageName .C.Message }}Filter is an autogenerated struct that
	// is used in generic {{ MessageName .C.Message }} queries.
	type {{ MessageName .C.Message }}Filter struct {
		{{- $oFile := .C.Outfile -}}
		{{- $fields := .C.Fields -}}
		{{ range $fields.Names -}}
			{{- $field := ($fields.ByName .) -}}
			...
			{{- $qName := ($fields.QueryNameByFieldName .) -}}
			{{- if $qName -}}
				{{ $fName }} {{ Pointer (GoFieldType $oFile $field) }} ` + "`" + `db:"{{ $qName }}"` + "`" + `
			{{- end }}
		{{ end -}}
	}

	func (x *{{ MessageName .C.Message }}Filter) fromGeneric(y *{{ QualifiedDalType .C.Outfile .C.Message }}Filter) {
		{{- $fields := .C.Fields -}}
		{{ range $fields.Names -}}
			{{- $field := ($fields.ByName .) -}}
			{{- $fName := (GoFieldName $field) -}}
	...
	// {{ MessageName .C.Message }}Scanner is an autogenerated struct that
	...
	`
		...
		tmpl, err := template.New("definePostgresScanner").
			Funcs(template.FuncMap{
				"MessageName":      generator.MessageName,
				"QualifiedType":    generator.QualifiedType,
				"QualifiedDalType": generator.QualifiedKind,
				"GoFieldName":      generator.GoFieldName,
				"GoFieldType":      generator.GoFieldType,
