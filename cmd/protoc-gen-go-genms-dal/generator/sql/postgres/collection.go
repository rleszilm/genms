...
func New{{ .msg.GoIdent.GoName }}Collection(db sql.DB, config *{{ .msg.GoIdent.GoName }}Config) *{{ .msg.GoIdent.GoName }}Collection {
	register{{ toTitleCase .msg.GoIdent.GoName }}MetricsOnce.Do(register{{ toTitleCase .msg.GoIdent.GoName }}Metrics)
	
	return &{{ .msg.GoIdent.GoName }}Collection{
		Unimplemented{{ .msg.GoIdent.GoName }}Collection: &dal_{{ .file.GoPackageName }}.Unimplemented{{ .msg.GoIdent.GoName }}Collection{},
		db: db,
		config: config,
		queryAll: "SELECT {{ template "fields" $state }} FROM " + config.TableName,
		{{ range .opts.Queries -}}
			{{- if not .StubOnly -}}
				query{{ toTitleCase .Name }}: ` + "`" + `SELECT {{ template "fields" $state }} FROM ` + "`" + ` + config.TableName
				{{- if .Fields }} +
					` + "`" + `WHERE 
					{{ range .Fields }}
						{{- . }} = :{{ . }} AND
