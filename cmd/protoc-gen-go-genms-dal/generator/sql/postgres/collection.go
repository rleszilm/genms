...
func (c *Collection) defineDefaultQueries() error {
		...
		defer func() {
			...
			if err != nil {
				ctx, err = {{ .P.Tag }}.New(ctx,
					{{ .P.Tag }}.Insert({{ ToCamelCase (.C.Message.Name) }}QueryError, "{{ ToLower (.C.Message.Name) }}_insert"),
				)
			}

			ctx, err = {{ .P.Tag }}.New(ctx,
				{{ .P.Tag }}.Insert({{ ToCamelCase (.C.Message.Name) }}QueryName, "{{ ToLower (.C.Message.Name) }}_insert"),
			)
	...
	func (x *{{ .C.Message.Name }}Collection) DoUpsert(ctx {{ .P.Context }}.Context, arg interface{}) ({{ .P.SQL }}.Result, error) {
		...
		defer func() {
			...
			if err != nil {
				ctx, err = {{ .P.Tag }}.New(ctx,
					{{ .P.Tag }}.Upsert({{ ToCamelCase (.C.Message.Name) }}QueryError, "{{ ToLower (.C.Message.Name) }}_upsert"),
				)
			}

			ctx, err = {{ .P.Tag }}.New(ctx,
				{{ .P.Tag }}.Upsert({{ ToCamelCase (.C.Message.Name) }}QueryName, "{{ ToLower (.C.Message.Name) }}_upsert"),
			)
	...
	func (x *{{ .C.Message.Name }}Collection) All(ctx {{ .P.Context }}.Context) ([]*{{ QualifiedType .C.Outfile .C.Message }}, error) {
		filter := &{{ QualifiedDalType .C.Outfile .C.Message }}Filter{}
		return x.find(ctx, "all", x.queryAll, filter)
	}

	// Filter implements {{ QualifiedDalType .C.Outfile .C.Message }}Collection.Filter
	func (x *{{ .C.Message.Name }}Collection) Filter(ctx {{ .P.Context }}.Context, arg *{{ QualifiedDalType .C.Outfile .C.Message }}Filter) ([]*{{ QualifiedType .C.Outfile .C.Message }}, error) {
		...
		{{- range $fields.Names -}}
			{{- $field := ($fields.ByName .) -}}
			{{- $fName := (GoFieldName $field) -}}
			{{- $qName := ($fields.QueryNameByFieldName .) -}}
			{{- if $qName -}}
				if arg.{{ $fName }} != nil {
					fields = append(fields, "{{ $qName }} = :{{ $qName }}")
				}
			{{- end }}
		{{ end -}}
	...
	func (x *{{ .C.Message.Name }}Collection) find(ctx {{ .P.Context }}.Context, label string, query string, arg *{{ QualifiedDalType .C.Outfile .C.Message }}Filter) ([]*{{ QualifiedType .C.Outfile .C.Message }}, error) {
		var err error
		start := {{ .P.Time }}.Now()
		{{ .P.Stats }}.Record(ctx, {{ ToCamelCase (.C.Message.Name) }}Inflight.M(1))
		defer func() {
		...
		{{ .C.Message.Name }}s := []*{{ QualifiedType .C.Outfile .C.Message }}{}
		for rows.Next() {
	...
	`
		...
		tmpl, err := template.New("definePostgresDefaultQueries").
			Funcs(template.FuncMap{
				...
				"GoFieldName":      generator.GoFieldName,
...
func (c *Collection) defineNewCollection() error {
	...
	`
		...
		p := map[string]string{
			"Collection":  c.File.QualifiedPackageName(generator.DalPackagePath(c.File)),
			"GenMSSQL":    c.File.QualifiedPackageName("github.com/rleszilm/genms/sql"),
			"GenerateSQL": c.File.QualifiedPackageName("github.com/rleszilm/genms/cmd/protoc-gen-go-genms-dal/generator/sql"),
		}
...
func (x *{{ .C.Message.Name }}Scanner) {{ .C.Message.Name }}() *{{ .C.Message.QualifiedKind }} {
	return &{{ .C.Message.QualifiedKind }}{
		...
		{{ range $n := .C.Fields.Names -}}
			...
			{{- if not $f.Ignore -}}
				...
				{{ ToTitleCase $f.Name }}: {{ $f.ValueFromSQLNil }},
			{{- end }}
		{{ end -}}
	}
}
` /*
		...
		// {{ .C.Message.Name }}Writer is an autogenerated struct that
		...
		func (x *{{ .C.Message.Name }}Writer) From{{ .C.Message.Name }}(y *{{ QualifiedType .C.Outfile .C.Message }}) {
			{{ range .WriterValues -}}
				{{ . }}
			{{ end }}
		}
...
func (c *Collection) defineTemplateProvider() error {
	/*tmplSrc := `// {{ .C.Message.Name }}QueryTemplateProvider is an interface that returns the query templated that should be executed
